...
void DoublyLinkedList<T>::pop_back() {
    if (empty()) {
        throw std::out_of_range("Lista está vazia");
    }
...
bool DoublyLinkedList<T>::Iterator<U>::operator==(
    ...
    return node == other.node && end == other.end;  // Usando '==' para comparação e '&&' para 'E' lógico
}
template<class T>
template<class U>
bool DoublyLinkedList<T>::Iterator<U>::operator!=(
...
DoublyLinkedList<T>::Iterator<U>DoublyLinkedList<T>::Iterator<U>::operator+(
        size_t offset) const{ // nao sei qual operator é o correto
    auto it = *this;
